                        ...
                        <tr>
                          ...
                          <td class="text-center">
                            <div class="btn-group-vertical w-100">
                              <div class="btn-group">
                                <select class="form-control w-100 btn btn-<?= $data['driver'] == "" ? "danger" : "primary"; ?> btn-sm font-size-14" id="shipment-driver-btn-<?= $data['id'] ?>" value="<?= $data['id'] ?? '' ?>" onchange="updateShipmentDriver('<?= $data['id'] ?>', this.value);" data-bs-toggle="tooltip" data-bs-placement="top" title="Driver">
                              ...
                              <?php 
                                if($data["receiver_address"]=='ECOMMERCE DELIVERY')
                                {
                                  if($data["quantity"]<3){
                                    ...
                                    $commission1 = 3;
                                  }elseif($data["quantity"]==4){
                                    $commission1 = 4;
                                  }elseif($data["quantity"]>=5){
                                    $commission1 = 5;
                                  }

                                  if($data["status"]=='info-received'){
                                    $commission2 = 10;
                                  }

                                  $tot_commission = $commission1 + $commission2;

                                }elseif($data["receiver_address"]=='BYSEA')
                                {
                                  if($data["weight"]>=1000 and $data["weight"]<=2000 and $data["service_level"] = 'standard'){
                                    $tot_commission = 2;
                                  }elseif($data["weight"]>=1000 and $data["weight"]<=2000){
                                    $tot_commission = 20;
                                  }elseif($data["weight"]>=3000){
                                    $tot_commission = 30;
                                  }

                                }elseif($data["receiver_address"]=='BS' or $data["receiver_address"]=='SE')
                                {
                                  $tot_commission = 1;
                                }
                          ...
                          <td>
                            ...
                            <?php if (session()->role == 0) : ?>
                              <div class="d-flex flex-row justify-content-center gap-2 mt-1 w-100 align-items-center">
                                <a href="/update_pickup/<?= $data['id'] ?>" class="d-flex text-info flex-row gap-1 align-items-center">
                                  <i class="fa fa-pencil-alt"></i>
                                  <?php echo lang('Edit') ?>
                                </a>
                                <form action="/delete_pickup/<?= $data['id'] ?>" method="post" onsubmit="return confirm('Delete this data?')">
                                  <input type="hidden" name="type" value="<?= $data['shipment_status'] ?>">
                                  <button type="submit" class="btn p-0 btn-link d-flex text-danger flex-row gap-1 align-items-center">
